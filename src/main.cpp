...
int main() {
  ...
  h.onMessage([&speed_limit, &map_waypoints_x,&map_waypoints_y,&map_waypoints_s, &my_car,
                &vel_control,
      ...
      if (s != "") {
        ...
        if (event == "telemetry"){
          // j[1] is the data JSON object
            ...
            // Lane change
            lane = my_car.next_lane(sensor_fusion, lane, car_s, end_path_s, prev_size);
            ...
            // Trajectory Planning
            // list of widely spaced (x,y) waypoints. Spaced 30m
            vector<double> ptsx;
            vector<double> ptsy;


            // reference x, y, yaw states.  Reference the starting point of car or ref at previous paths end point
            double ref_x = car_x;
            double ref_y = car_y;
            ...
            if (prev_size < 2)
            {
              // generate 2 points making the path tangent to our car
            ...
            else //use previous path end point start as ref.
            {
              ...
              //use 2 points that make the path tangent to the previous path end point
            ...
            // Adding spaced points ahead of the starting reference (Frenet)
            ...
            for (int i = 0; i < ptsx.size(); i++)
            {
              ...
              //change car ref angle 0 degrees - our car local coordinates
            ...
            // path planner output 50 points
            for (int i = 1; i <= 50-previous_path_x.size(); i++)
            {
              double N = (target_dist/(0.02*ref_vel/2.24)); // switch to m/s
